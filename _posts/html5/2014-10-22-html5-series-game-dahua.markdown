...
function initSprites(images,callback){
  ...
  for (var i=0;i<100;++i){ //随机创建任务数
    sprites[i] = {
      x : Math.random() * (bg.width-padding) + padding/2,
      y : Math.random() * (bg.height-padding) + padding/2,
...
var drawFrame = function(images){
  ...
  for (var i=0,len=sprites.length;i<len;++i){ // 遍历人物
    ...
    s.x += Math.random() * 4 - 2;
    s.y += Math.random() * 4 - 2;
    if (s.x+size.w >= bg.width) s.x -= 10;
    if (s.y+size.h >= bg.height) s.y -= 10;
